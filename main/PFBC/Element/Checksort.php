...
JS;
	...
	public function render() { 
		...
		foreach($this->options as $value => $text) {
			...
			echo '<div class="pfbc-checkbox"><table summary="CheckButton" cellpadding="0" cellspacing="0"><tr><td valign="top"><input id="', $this->attributes["id"], "-", $count, '"', $this->getAttributes(array("id", "value", "checked", "name", "onclick")), ' value="', $this->filter($value), '"';
...
if(typeof updateChecksort != "function") {		
	function updateChecksort(element, text) {
		...
		if(element.checked)
		{
				jQuery("#" + id + " ul").append('<li id="' + id + '-sort-' + index + '" class="ui-state-default">'+
